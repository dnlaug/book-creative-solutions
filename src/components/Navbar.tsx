...
const Navbar = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300 py-4 px-4 sm:px-6 md:px-10", scrolled ? "bg-white/80 shadow-sm backdrop-blur-card" : "bg-transparent")}>
      ...
      <div className={cn("fixed inset-0 bg-white/95 backdrop-blur-md z-40 flex flex-col items-center justify-center transition-transform duration-300 ease-in-out md:hidden", isOpen ? "translate-x-0" : "translate-x-full")}>
        <nav className="flex flex-col items-center space-y-8">
          {navItems.map(item => <a key={item.href} href={item.href} className="text-2xl font-labrador transition-colors hover:text-primary" onClick={e => {
          e.preventDefault();
          handleNavLinkClick(item.href);
        }}>
              {item.label}
