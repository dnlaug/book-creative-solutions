  ...
  return <section id="cases" className="section bg-gray-50">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <AnimatedElement direction="up" className="flex flex-wrap justify-center gap-2 mb-12">
          {categories.map((category, index) => <button key={index} onClick={() => setActiveCategory(category)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 font-labrador ${activeCategory === category ? "bg-brand-pink text-white shadow-md" : "bg-white hover:bg-gray-100 text-gray-600"}`}>
              {category}
            </button>)}
        </AnimatedElement>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 transition-opacity duration-300 ${isFiltering ? "opacity-50" : "opacity-100"}`}>
          {filteredProjects.map((project, index) => <AnimatedElement key={project.title} direction="up" delay={project.delay} className="group portfolio-item">
              <div className="relative overflow-hidden rounded-xl hover-shadow">
                ...
                <div className="portfolio-overlay">
                  ...
                  <h3 className="text-xl font-mairy text-white mt-1">
                    {project.title}
                  </h3>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {project.tags.map((tag, tagIndex) => <span key={tagIndex} className="inline-block bg-white/20 text-white text-xs px-2 py-1 rounded-full">
                        {tag}
                      </span>)}
                  </div>
          ...
          <div className="bg-white rounded-2xl p-8 md:p-12 shadow-sm border-t-4 border-brand-orange">
            ...
            <div className="relative max-w-4xl mx-auto">
              <div className="overflow-hidden">
                <div className={`transition-all duration-500 ease-in-out ${isAnimating ? 'opacity-0' : 'opacity-100'}`} style={{
                transform: isAnimating ? 'scale(0.96)' : 'scale(1)'
                  ...
                  <div className="relative p-8 md:p-10 bg-gray-50 rounded-2xl border border-gray-100">
                    <div className="absolute top-6 left-6 text-brand-green/20">
                      <Quote size={48} className="rotate-180 opacity-20 text-accent" />
                    </div>
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-center gap-4">
                        <img src={testimonials[currentIndex].image} alt={testimonials[currentIndex].author} className="w-12 h-12 rounded-full object-cover" />
                        <div>
              ...
              <div className="flex justify-center mt-8 gap-4">
                {testimonials.map((_, idx) => <button key={idx} onClick={() => setCurrentIndex(idx)} className={`w-2.5 h-2.5 rounded-full transition-all ${currentIndex === idx ? 'bg-brand-pink scale-125' : 'bg-gray-300'}`} aria-label={`Go to testimonial ${idx + 1}`} />)}
              </div>
              
              <div className="absolute top-1/2 -translate-y-1/2 left-0 -ml-4 md:-ml-6">
                <button onClick={prevTestimonial} className="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-700 hover:bg-brand-pink hover:text-white transition-colors" aria-label="Previous testimonial">
                  <ArrowLeft size={18} />
                </button>
              </div>
              
              <div className="absolute top-1/2 -translate-y-1/2 right-0 -mr-4 md:-mr-6">
                <button onClick={nextTestimonial} className="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gray-700 hover:bg-brand-pink hover:text-white transition-colors" aria-label="Next testimonial">
                  <RightArrow size={18} />
                </button>
              </div>
            </div>
